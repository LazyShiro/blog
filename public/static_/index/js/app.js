var e = {
	...
	favicon: {
		...
		hidden: "index/images/failure.ico"
	},
	darkmode: false,
	auto_scroll: true,
	js: {
	...
	valine: {
		...
		tagMember: {
			master: ["masterEmailMd5"],
			friend: ["055a933d93289954b5391d3ef19a0cca"],
			investor: []
		}
	},
	quicklink: {
	...
	audio: [
		{
			title: "musicTitle1",
			list: ["musicListLink1"]//https://music.163.com/#/playlist?id=3065220035
		},
		{
			title: "musicTitle2",
			list: ["musicListLink2"]
...
const i = function (t, a) {
	var n = e[t][a];
	if (n.indexOf("npm") > -1 || n.indexOf("gh") > -1 || n.indexOf("combine") > -1) return "//cdn.jsdelivr.net/" + n;
	if (n.indexOf("http") > -1) return n;
	return m + n
};
...
const c = function (e, t, a) {
	var n = {
		targets: typeof t == "number" ? e.parentNode : document.scrollingElement, duration: 500, easing: "easeInOutQuad", scrollTop: t || (typeof e == "number" ? e : e ? e.top() + document.documentElement.scrollTop - R : 0), complete: function () {
			a && a()
		}
	};
	anime(n)
	...
	switch (t) {
		...
		case"bounceUpIn":
			n = {
				begin: function (t) {
					e.display("block")
				}, translateY: [{value: -60, duration: 200}, {value: 10, duration: 200}, {value: -5, duration: 200}, {value: 0, duration: 200}], opacity: [0, 1]
			};
			i = "block";
			break;
		case"shrinkIn":
...
var f = null;
const h = /mobile/i.test(window.navigator.userAgent);
const p = function (e, t) {
	var a = {
		type: "audio", mode: "random", btns: ["play-pause", "music"], controls: ["mode", "backward", "play-pause", "forward", "volume"], events: {
		...
		random: function (e) {
			...
			var n = Math.floor((e - a * 3600) / 60);
			var i = Math.floor(e - a * 3600 - n * 60);
			return (a > 0 ? [a, n, i] : [n, i]).map(t).join(":")
		}, nameMap: {dragStart: h ? "touchstart" : "mousedown", dragMove: h ? "touchmove" : "mousemove", dragEnd: h ? "touchend" : "mouseup"}
	}, i = null;
	e.player = {
	...
	var r = {
		el: null, create: function () {
			if (this.el) return;
			this.el = e.createChild("div", {className: "player-info", innerHTML: (e.player.options.type == "audio" ? '<div class="preview"></div>' : "") + '<div class="controller"></div><div class="playlist"></div>'}, "after");
	...
	var s = {
		el: null, data: [], index: -1, errnum: 0, add: function (e, t) {
			...
			this.data.map(function (a, n) {
				if (a.el) return;
				var r = "list-" + e.player._id + "-" + a.group;
				var o = u("#" + r);
				if (!o) {
				...
				a.el = o.child("ol").createChild("li", {
					title: a.name + " - " + a.artist, innerHTML: '<span class="info"><span>' + a.name + "</span><span>" + a.artist + "</span></span>", onclick: function (t) {
			...
			var e = this.current();
			var t = this.el.child("li.active");
			t && t.removeClass("active");
			var a = this.el.child(".tab.active");
	...
	var o = {
		el: null, data: null, index: 0, create: function (e) {
			...
			if (e) {
				...
				const n = t.length;
				for (var i = 0; i < n; i++) {
					const r = t[i].match(/\[(\d{2}):(\d{2})(\.(\d{2,3}))?]/g);
					const s = t[i].replace(/.*\[(\d{2}):(\d{2})(\.(\d{2,3}))?]/g, "").replace(/<(\d{2}):(\d{2})(\.(\d{2,3}))?>/g, "").replace(/^\s+|\s+$/g, "");
					if (r) {
						const o = r.length;
						for (var c = 0; c < o; c++) {
							const l = /\[(\d{2}):(\d{2})(\.(\d{2,3}))?]/.exec(r[c]);
							const d = l[1] * 60;
							const u = parseInt(l[2]);
							const f = l[4] ? parseInt(l[4]) / ((l[4] + "").length === 2 ? 100 : 1e3) : 0;
							const h = d + u + f;
							a.push([h, s])
						}
					}
	...
	var p = {
		el: null, bar: null, create: function () {
			...
			var a = ((e.clientX || e.changedTouches[0].clientX) - t.left()) / t.width();
			a = Math.max(a, 0);
			return Math.min(a, 1)
		}, drag: function (t) {
			t.preventDefault();
...
var m = e.statics.indexOf("//") > 0 ? e.statics : e.root;
...
const L = u("#nav");
const E = u("#header");
const T = L.child(".toggle");
const M = u("#quick");
const N = u("#sidebar");
const O = u("#brand");
var j = u("#tool"), I, H, S, q;
var A = u("#search");
var R, P, _;
var D = window.innerHeight;
var z = window.innerWidth;
var B = 0, W = window.location.href;
var Y;
const X = lozad("img, [data-background-image]", {
	loaded: function (e) {
		e.addClass("lozaded")
	}
});
...
const U = function () {
	...
	u(".theme").addEventListener("click", function (e) {
		var t = e.currentTarget.child(".ic");
		var a = w.createChild("div", {id: "neko", innerHTML: '<div class="planet"><div class="sun"></div><div class="moon"></div></div><div class="body"><div class="face"><section class="eyes left"><span class="pupil"></span></section><section class="eyes right"><span class="pupil"></span></section><span class="nose"></span></div></div>'});
		var n = function () {
...
const K = function (e) {
	if (!e) return;
	var t = w.createChild("div", {innerHTML: e, className: "tip"});
	setTimeout(function () {
		t.addClass("hide");
		setTimeout(function () {
			w.removeChild(t)
...
const Q = function (e) {
	...
	var n = a();
	var i = n > t ? n - t : document.body.scrollHeight - t;
	var r = window.pageYOffset > P;
	...
	v.y = window.pageYOffset;
	var o = Math.round(Math.min(100 * window.pageYOffset / i, 100)) + "%";
...
const te = function (e, t) {
	var a = w.createChild("textarea", {style: {top: window.scrollY + "px", position: "absolute", opacity: "0"}, readOnly: true, value: e});
	const n = document.getSelection();
	const i = n.rangeCount > 0 ? n.getRangeAt(0) : false;
	a.select();
...
const ne = function () {
	...
	var a = document.createElement("ul"), n = "active";
	a.className = "tab";
	["contents", "related", "overview"].forEach(function (e) {
		var t = N.child(".panel." + e);
		...
		var s = document.createTextNode(t.attr("data-title"));
		r.appendChild(s);
		i.appendChild(r);
		i.addClass(e + " item");
		if (n) {
...
const ie = function () {
	...
	a = a.map(function (t, a) {
		var i = t.child("a.toc-link");
		var s = u(decodeURI(i.attr("href")));
		if (!s) return;
		var o = s.child("a.anchor");
		var l = function (e) {
			e.preventDefault();
			var t = u(decodeURI(e.currentTarget.attr("href")));
			n = a;
			c(t, null, function () {
				r(a);
				n = null
	...
	var r = function (e, n) {
		...
		while (!s.matches(".contents")) {
			if (s.matches("li")) {
				s.addClass("active");
				var o = u(s.child("a.toc-link").attr("href"));
				if (o) {
					o.addClass("active")
				}
			}
...
const ce = function () {
	u.each(".menu .item:not(.title)", function (t) {
		var a = t.child("a[href]");
		if (!a) return;
		var n = a.pathname === location.pathname || a.pathname === location.pathname.replace("index.html", "");
		var i = !e.root.startsWith(a.pathname) && location.pathname.startsWith(a.pathname);
		var r = a.hostname === location.hostname && (n || i);
...
const ue = function (e) {
	if (window.location.pathname.indexOf('friend') > -1) {
		return;
	}
	if (u(e + " .md img")) {
		s("fancybox");
		r("fancybox", function () {
			...
			u.each(e + " .md img:not(.emoji):not(.vemoji)", function (e) {
				var a = t(e);
				var n, i = "image-info";
				if (!a.is("a img")) {
					var r = a.attr("data-src") || a.attr("src");
					a.data("safe-src", r);
					var s = a.wrap('<a class="fancybox" href="' + r + '" itemscope itemtype="http://schema.org/ImageObject" itemprop="url"></a>').parent("a");
					if (!a.is(".gallery img")) {
						s.attr("data-fancybox", "default").attr("rel", "default")
					} else {
						i = "jg-caption"
					}
				}
				if (n = e.attr("title")) {
					s.attr("data-caption", n);
			...
			u.each(e + " div.gallery", function (e, a) {
				t(e).justifiedGallery({rowHeight: t(e).data("height") || 120, rel: "gallery-" + a}).on("jg.complete", function () {
					t(this).find("a").each(function (e, t) {
						t.attr("data-fancybox", "gallery-" + a)
					})
				})
			});
			t.fancybox.defaults.hash = false;
			t(e + " .fancybox").fancybox({loop: true, helpers: {overlay: {locked: false}}})
		}, window.jQuery)
	}
};
const fe = function () {
	...
	u(".post.block").oncopy = function (e) {
		...
		if (window.getSelection().toString().length > 30 && t) {
			e.preventDefault();
			var a = "# " + t.child(".author").innerText;
			var n = "# " + t.child(".link").innerText;
			var i = "# " + t.child(".license").innerText;
			var r = a + "<br>" + n + "<br>" + i + "<br><br>" + window.getSelection().toString().replace(/\r\n/g, "<br>");
	...
	u.each("figure.highlight", function (e) {
		var t = e.child(".code-container");
		var a = e.child("figcaption");
...
const me = function (t) {
	...
	if (!A) {
		A = w.createChild("div", {id: "search", innerHTML: '<div class="inner"><div class="header"><span class="icon"><i class="ic i-search"></i></span><div class="search-input-container"></div><span class="close-btn"><i class="ic i-times-circle"></i></span></div><div class="results"><div class="inner"><div id="search-stats"></div><div id="search-hits"></div><div id="search-pagination"></div></div></div></div>'})
	}
	var a = instantsearch({
		indexName: e.search.indexName, searchClient: algoliasearch(e.search.appID, e.search.apiKey), searchFunction: function (e) {
			var t = u(".search-input");
			if (t.value) {
				e.search()
			}
		}
	...
	a.addWidgets([instantsearch.widgets.configure({hitsPerPage: e.search.hits.per_page || 10}), instantsearch.widgets.searchBox({container: ".search-input-container", placeholder: LOCAL.search.placeholder, showReset: false, showSubmit: false, showLoadingIndicator: false, cssClasses: {input: "search-input"}}), instantsearch.widgets.stats({
		container: "#search-stats", templates: {
			text: function (e) {
				var t = LOCAL.search.stats.replace(/\$\{hits}/, e.nbHits).replace(/\$\{time}/, e.processingTimeMS);
		...
		container: "#search-hits", templates: {
			item: function (t) {
				var a = t.categories ? "<span>" + t.categories.join('<i class="ic i-angle-right"></i>') + "</span>" : "";
	...
	}), instantsearch.widgets.pagination({container: "#search-pagination", scrollTo: false, showFirst: false, showLast: false, templates: {first: '<i class="ic i-angle-double-left"></i>', last: '<i class="ic i-angle-double-right"></i>', previous: '<i class="ic i-angle-left"></i>', next: '<i class="ic i-angle-right"></i>'}, cssClasses: {root: "pagination", item: "pagination-item", link: "page-number", selectedItem: "current", disabledItem: "disabled-item"}})]);
	a.start();
...
const ve = function () {
	...
	M.child(".down").addEventListener("click", se);
	M.child(".up").addEventListener("click", re);
	if (!j) {
		j = E.createChild("div", {id: "tool", innerHTML: '<div class="item player"></div><div class="item contents"><i class="ic i-list-ol"></i></div><div class="item chat"><i class="ic i-comments"></i></div><div class="item back-to-top"><i class="ic i-arrow-up"></i><span>0%</span></div>'})
	}
	I = j.child(".player");
...
const be = function () {
	ve();
	Y = new Pjax({selectors: ["head title", ".languages", ".pjax", "script[data-config]"], analytics: false, cacheBust: false});
...
console.log('%c他们又故意的高声嚷道,"你一定又偷了人家的主题了!"\r\n博主睁大眼睛说,"你怎么这样凭空污人清白..."\r\n"什么清白?我前天亲眼见你偷了何家的主题,吊着打."\r\n博主便涨红了脸,额上的青筋条条绽出,争辩道,"窃主题不能算偷...窃主题！...IT人的事,能算偷么?"\r\n说话间,从袜子里掏出来一串神秘代码2020****2000400111008200****8049\r\n接连便是难懂的话,什么"君子固穷",什么"者乎"之类,引得众人都哄笑起来\r\n屏幕内外充满了快活的空气.', "color: white; background: #666; padding:5px;");
...
function Se(e, t) {
	...
	}).add({targets: a, radius: anime.random(80, 160), lineWidth: 0, alpha: {value: 0, easing: "linear", duration: anime.random(600, 800)}, duration: anime.random(1200, 1800), easing: "easeOutExpo", update: He}, 0)
}
