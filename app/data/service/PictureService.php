...
{
	protected $pictureCategoryTable = 'DataPictureCategory';
	 ...
	 * @return array
	 */
	public function buildListState(array &$list): array
	{
		if (count($list) > 0) {
			foreach ($list as &$vo) {
				$category = str2arr($vo['category']);

				$vo['category'] = $this->app->db->name($this->pictureCategoryTable)->where([['status', '=', 1], ['deleted', '=', 0], ['id', 'in', $category]])->column('name,remark');
			}
		}
		return $list;
	}
